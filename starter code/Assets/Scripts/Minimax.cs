...
public class Minimax : MonoBehaviour
{
    ...
    public MoveData GetMove()   // Sets best move to tile 0,0
    {
    ...
    int CalculateMinMax(int depth, int alpha, int beta, bool max)   // Signature of change
    {
        GetBoardState();    // Where the pieces are and the state of the board
        ...
        if (max)
        {
            ...
            allMoves = Shuffle(allMoves);   // Added to insert randomness for maxscore
            ...
            return alpha;   // Instead of maxScore
        }
        else
        {
            ...
            allMoves = Shuffle(allMoves);   // Added to include randomness for minscore
            foreach (MoveData move in allMoves)
            {
                moveStack.Push(move);
            ...
            return beta;    // Instead of minScore
        }
    }

    public List<T> Shuffle<T>(List<T> list)  // Randomize
    {  
        int n = list.Count;  
        while (n > 1) {  
            n--;  
